...
// HardReset simulates power loss: SIGKILLs qemu (no SIGTERM grace) and
// relaunches against existing disk state.
func (q *QEMU) HardReset(ctx context.Context) (context.Context, error) {
...
// HardPowerCycle is retained for compatibility. New callers should use
// HardReset through the matcher capability.
func (q *QEMU) HardPowerCycle(ctx context.Context) (context.Context, error) {
	return q.HardReset(ctx)
}
